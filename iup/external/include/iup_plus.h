...
namespace Iup
{
  ...
  inline int IsOpened() { return IupIsOpened(); }
  inline void PostMessage(const Element& elem, const char* s, int i, double d, void* p);

  inline int Execute(const char* filename, const char* parameters) { return IupExecute(filename, parameters); }
  inline int ExecuteWait(const char* filename, const char* parameters) { return IupExecuteWait(filename, parameters); }
  inline int Help(const char* url) { return IupHelp(url); }
  inline void Log(const char* type, const char* str) { IupLog(type, "%s", str); }

  inline int StringCompare(const char* str1, const char* str2, int casesensitive, int lexicographic) { return IupStringCompare(str1, str2, casesensitive, lexicographic); }
  ...
  inline void PostMessage(const Element& elem, const char* s, int i, double d, void* p) { IupPostMessage(elem.GetHandle(), s, i, d, p); }
  ...
  class Tray : public Element
  {
  public:
    Tray() : Element(IupTray()) {}
    Tray(Ihandle* _ih) : Element(_ih) {}
    Tray(const Element& elem) : Element(elem.GetHandle()) {}
  };
  class Notify : public Element
  {
  public:
  ...
  public:
    ...
    void DrawRoundedRectangle(int x1, int y1, int x2, int y2, int corner_radius) { IupDrawRoundedRectangle(ih, x1, y1, x2, y2, corner_radius); }
    void DrawArc(int x1, int y1, int x2, int y2, double a1, double a2) { IupDrawArc(ih, x1, y1, x2, y2, a1, a2); }
    void DrawEllipse(int x1, int y1, int x2, int y2) { IupDrawEllipse(ih, x1, y1, x2, y2); }
    void DrawPolygon(int* points, int count) { IupDrawPolygon(ih, points, count); }
    void DrawPixel(int x, int y) { IupDrawPixel(ih, x, y); }
    void DrawBezier(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4) { IupDrawBezier(ih, x1, y1, x2, y2, x3, y3, x4, y4); }
  ...
  class DatePick : public Control
  {
  public:
    DatePick() : Control(IupDatePick()) {}
    DatePick(Ihandle* _ih) : Control(_ih) {}
    DatePick(const Element& elem) : Control(elem.GetHandle()) {}
  };
  class Calendar : public Control
  {
  public:
    Calendar() : Control(IupCalendar()) {}
    Calendar(Ihandle* _ih) : Control(_ih) {}
    Calendar(const Element& elem) : Control(elem.GetHandle()) {}
  };
  ...
  {
  public:
    Scrollbar(const char* type = 0) : Control(IupScrollbar(type)) {}
    Scrollbar(Ihandle* _ih) : Control(_ih) {}
    Scrollbar(const Element& elem) : Control(elem.GetHandle()) {}
  };
  ...
  {
  public:
    Popover() : Container(IupPopover(0)) {}
    Popover(Control child) : Container(IupPopover(child.GetHandle())) {}
    Popover(const Popover& container) : Container(container.GetHandle()) {}
    Popover(Ihandle* _ih) : Container(_ih) {}
  };
  ...
  {
  public:
    Table() : Control(IupTable()) {}
    Table(Ihandle* _ih) : Control(_ih) {}
    Table(const Element& elem) : Control(elem.GetHandle()) {}
  };
  ...
  public:
    ...
    int FindSegment(double cnv_x, double cnv_y, int &ds_index, int &sample_index1, int &sample_index2) { return IupPlotFindSegment(ih, cnv_x, cnv_y, &ds_index, &sample_index1, &sample_index2); }
  };
