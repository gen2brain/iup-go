...
static void iDrawSetSource(IdrawCanvas* dc, const IupDrawSource* src)
{
  ...
  else if (src->type == IUP_SOURCE_RADIAL_GRADIENT)
  {
    ...
    cairo_set_source_rgba(dc->image_cr, iupgtk4ColorToDouble(iupDrawRed(src->color)), iupgtk4ColorToDouble(iupDrawGreen(src->color)), iupgtk4ColorToDouble(iupDrawBlue(src->color)), iupgtk4ColorToDouble(iupDrawAlpha(src->color)));
}
