...
static int cocoaDropCallbackWithImage(NSImage* ns_image, IFnsViii drop_data_callback, Ihandle* ih, NSPoint drop_point_iup)
{
  int w, h, bpp;
  ...
  int ret_val = drop_data_callback(ih, type_string, (void*)img_data, (int)buffer_size, (int)drop_point_iup.x, (int)drop_point_iup.y);
...
int cocoaTargetDropBasePerformDropCallback(Ihandle* ih, id<NSDraggingInfo> the_sender, NSPasteboard* paste_board, NSPoint drop_point)
{
  ...
  for(id drop_item in acceptable_drop_items)
  {
    ...
    {
      ret_val = cocoaDropCallbackWithImage((NSImage*)drop_item, drop_data_callback, ih, drop_point_iup);
    }
    ...
    else
    {
      ...
      if([best_type isEqualToString:NSPasteboardTypePNG] || [best_type isEqualToString:NSPasteboardTypeTIFF])
      {
        ...
        ret_val = cocoaDropCallbackWithImage(ns_image, drop_data_callback, ih, drop_point_iup);
      }
...
#pragma clang diagnostic pop
    ...
    else
    {
      ...
      [pasteboard_item setDataProvider:self forTypes:registered_types];
...
- (NSDraggingItem*) defaultDraggingItem
{
  ...
  if(nil != pasteboard_item)
  {
    ...
    else if([self useAutoGenerateDragImage])
    {
      NSRect bounds_rect = [main_view bounds];
      NSData* pdf_data = [main_view dataWithPDFInsideRect:bounds_rect];
      NSImage* image_data = [[NSImage alloc] initWithData:pdf_data];
...
static int cocoaTargetDropSetDropTypesAttrib(Ihandle* ih, const char* value)
{
  ...
  if(NULL != value)
  {
    NSMutableArray* array_of_types = cocoaParseDragDropTypes(value);
...
static int cocoaSourceDragSetDragTypesAttrib(Ihandle* ih, const char* value)
{
  ...
  if(value)
  {
    ...
    /* Expand FILEPROMISE into actual file promise types */
    NSUInteger idx = [array_of_types indexOfObject:@"FILEPROMISE"];
    if (idx != NSNotFound)
    {
...
void iupdrvRegisterDragDropAttrib(Iclass* ic)
{
  ...
  iupClassRegisterAttribute(ic, "DRAGTYPES", NULL, cocoaSourceDragSetDragTypesAttrib, NULL, NULL, IUPAF_NOT_MAPPED|IUPAF_NO_INHERIT);
  ...
  iupClassRegisterAttribute(ic, "DROPTYPES", NULL, cocoaTargetDropSetDropTypesAttrib, NULL, NULL, IUPAF_NOT_MAPPED|IUPAF_NO_INHERIT);
