...
**EXTFILTER** [Windows and GTK Only]: Defines several file filters. It has priority over FILTERINFO and FILTER.
...
**FILTERINFO** [Windows and GTK Only]: Filter's description.
If not defined the filter itself will be used as its description.

**FILTERUSED** [Windows and GTK Only]: the index of the filter in EXTFILTER to use starting at 1.
...
Default is "NO", i.e., prompt before overwrite.  (GTK 2.8)
...
**SHOWHIDDEN**: Show hidden files. Default: NO. (GTK 2.6)
...
[WHEEL_CB](../call/iup_wheel_cb.md): Action generated when the mouse wheel is rotated over the preview canvas.  NOTICE: NOT working in Windows 7.
...
In Windows, if FILE_CB and HELP_CB are not defined, and x,y are IUP_CENTER or IUP_CURRENT then it will use a newer Explorer interface available since Windows Vista.
...
In GTK uses gtk_file_chooser_dialog, in Windows uses GetOpenFileName/GetSaveFileName, and in Motif uses xmFileSelectionBox.
