...
It is called before the BRANCH*_CB callbacks.
...
**drop_id**: Identifier of the clicked node where the drop was executed. -1 indicates a drop in a blank area.\
...
It is only a notification, the action cannot be aborted.
No node dependent attribute can be consulted during the callback. Not called when the tree is unmapped.
...
[BUTTON_CB](../call/iup_button_cb.md): Action generated when any mouse button is pressed or released inside the element.
Use [IupConvertXYToPos](../func/iup_convertxytopos.md) to convert (x,y) coordinates in the node identifier.

[MOTION_CB](../call/iup_motion_cb.md): Action generated when the mouse is moved over the element.
Use [IupConvertXYToPos](../func/iup_convertxytopos.md) to convert (x,y) coordinates in the node identifier.
...
Also in Motif, some LEAVEWINDOW_CB events are delayed to when the user enters again, firing a leave and enter events at enter time.
