...
func main() {
	...
	// Register images for use in a virtual list
	iup.ImageRGBA(32, 32, imgGopher).SetHandle("icon1")
	iup.ImageRGBA(32, 32, imgTecgraf).SetHandle("icon2")

	// Simulate a large dataset with alternating images
	dataset = make([]string, 100000)
	images = make([]string, 100000)
	for i := 0; i < 100000; i++ {
		dataset[i] = fmt.Sprintf("Item %d - This is the description for item number %d", i+1, i+1)
		// Alternate between images, some items have no image
		switch i % 3 {
		case 0:
			images[i] = "icon1"
		case 1:
	...
	// Create a list in virtual mode (plain list, no dropdown, no editbox)
	list := iup.List()

	// Enable virtual mode BEFORE setting item count
	list.SetAttribute("VIRTUALMODE", "YES")

	// Enable images in the list
	list.SetAttribute("SHOWIMAGE", "YES")
	...
	// Set VALUE_CB callback to provide text data on demand
	...
	// Set IMAGE_CB callback to provide image data on demand
	iup.SetCallback(list, "IMAGE_CB", iup.ListImageFunc(func(ih iup.Ihandle, pos int) string {
		// pos is 1-based
	...
	iup.SetCallback(list, "ACTION", iup.ListActionFunc(func(ih iup.Ihandle, text string, item, state int) int {
		if state == 1 {
			...
			imgName := ""
			if item > 0 && item <= len(images) {
				imgName = images[item-1]
				if imgName == "" {
					imgName = "(none)"
				}
			}
			status.SetAttribute("TITLE", fmt.Sprintf("Selected item %d (image: %s)", item, imgName))
		}
		return iup.DEFAULT
	}))
	...
	// Info label
	lblInfo := iup.Label("Virtual mode with images: 100,000 items loaded instantly!\n" +
		"Text is fetched via VALUE_CB, images via IMAGE_CB.\n" +
		"Items alternate: icon1, icon2, no icon.\n" +
	...
	dlg := iup.Dialog(
		iup.Vbox(
			iup.Label("IupList Virtual Mode with Images").SetAttributes(`FONT="Sans, Bold 12"`),
			lblInfo,
			list,
			iup.Hbox(
				btnChangeCount,
	...
	dlg.SetAttributes(`TITLE="IupList Virtual Mode with Images"`)
